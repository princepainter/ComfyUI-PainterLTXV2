...
class PainterLTX2VPlus:
    """Combined LTXV latent generator with first/last frame control and upscaling support"""
    ...
    RETURN_TYPES = ("LATENT", "LATENT", "LATENT", "INT", "INT")
    RETURN_NAMES = ("latent", "video_latent", "audio_latent", "width", "height")
    FUNCTION = "execute"
    CATEGORY = "latent/video/ltxv"
    DESCRIPTION = "Generate LTXV latent with optional first/last frame control and upscaling"

    def execute(self, video_vae, width=768, height=512, length=97, frame_rate=25.0,
                batch_size=1,
                audio_vae=None, start_image=None, end_image=None, latent=None,
                upscale_model=None):
        
        # Hardcoded strength values (previously exposed as parameters)
        first_strength = 1.0
        last_strength = 1.0
        
        # Step 1: Prepare base latent tensor
        ...
        # Step 3: Apply upscaling if model provided
        if upscale_model is not None:
            samples = self._apply_upscale(samples, upscale_model, video_vae)
            noise_mask = None  # Remove mask after upsampling as per original logic

        # Step 4: Build main output dictionary
        output = {"samples": samples}
        if noise_mask is not None:
            output["noise_mask"] = noise_mask

        # Step 5: Add audio latent if audio_vae provided
        if audio_vae is not None:
            output = self._attach_audio_latent(output, audio_vae, length, frame_rate, batch_size)

        # Step 6: Split and return A/V outputs plus dimensions
        return self._split_av_outputs(output, audio_vae is not None, width, height)
    ...
    def _apply_upscale(self, samples, upscale_model, video_vae):
        """Upsample latents using provided model with VAE statistics"""
        ...
        try:
            ...
            # Unnormalize, upsample, then normalize back
            ...
            return upsampled.to(dtype=input_dtype, device=comfy.model_management.intermediate_device())
        finally:
            upscale_model.cpu()
    ...
    def _split_av_outputs(self, output, has_audio, width, height):
        """Split combined A/V latent into separate outputs plus dimensions"""
        main_latent = output
        ...
        if has_audio and isinstance(output["samples"], comfy.nested_tensor.NestedTensor):
            ...
            if len(latents) >= 2:
                ...
                return (main_latent, video_latent, audio_latent, width, height)
